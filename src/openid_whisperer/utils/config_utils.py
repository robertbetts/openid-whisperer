...
def load_environment_variables(env_target: str | None = None) -> None:
    ...
    if "PYTEST_CURRENT_TEST" in os.environ:
        logger.warning("Skipping the loading of the DotEnv file .env when pytest is running.")
    else:
        if not os.path.exists(".env"):
            logger.warning("DotEnv file .env not found in the current directory: %s", os.getcwd()) # pragma: no cover
        else:
    ...
    if not os_env_target and not env_target:
        env_target = "DEV"
        logger.warning("Defaulting target environment variable ENVIRONMENT to %s", env_target)
        os.environ["ENVIRONMENT"] = env_target
    elif not os_env_target:
        logger.warning("Defaulting os environment variable ENVIRONMENT to %s", env_target)
        os.environ["ENVIRONMENT"] = env_target
    elif not env_target:
        env_target = os_env_target
        logger.warning("Using target environment from variable ENVIRONMENT, %s", env_target)
    elif env_target.upper() != os_env_target.upper():  # check for collision prioritise env_target
        logger.warning("Overriding os environment variable ENVIRONMENT from %s to %s", os_env_target, env_target)
        os.environ["ENVIRONMENT"] = env_target
    else:
    ...
    if not os.path.exists(env_file_name):
        logger.warning("DotEnv file %s, not found in the current directory, %s", env_file_name, os.getcwd())
    else:
...
def get_bind_address(input_value: str) -> list[str]:
    ...
    if any([(":" not in item) or (not item.split(":")[-1].isdecimal()) for item in addresses]):
        logging.warning("Invalid bind address value, must be in the form (host|ip):port[,(host|ip):port] : %s",
                        input_value)
        return []
    else:
        return addresses
